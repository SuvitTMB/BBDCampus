...
async function main() {
  await liff.init({ liffId: "1655966947-PXjQ9b5J" });
...
function WaitingPage() {
  ...
  .get().then((snapshot)=> {
    ...
    if(xEmpType!="") {
      str +='<div class="profile-txt1" style="line-height: 1.2;color:#0056ff;">ยินดีต้อนรับสู่<br>Management Roadshow 2022<br><font color="#0056ff">'+xEmpType+'</font></div>';
    } else {
      str +='<div class="profile-txt1" style="line-height: 1.2;">ได้ลงทะเบียนเข้าร่วมงาน<br>Management Roadshow 2022<br><font color="#0056ff">'+sCampName+'</font></div>';
    }
    if(sDateTime!="") {
      str +='<div style="color:#999;font-size:11px;font-weight: 300;">ลงทะเบียนเมื่อ : '+ sDateTime +'</div>';
    }
    if(sessionStorage.getItem("ATKimg")!=null) {
      str +='<div class="btn-t4" onclick="showATK()" style="margin-top:10px;width:270px;">1. แสดงผล ATK ก่อนเข้างาน</div>';
    } else {
...
function showATK() {
  var str = "";
  str +='<div class="title_container"><div class="title-head">Management Roadshow 2022';
